...
import type { LoginInput, RegisterDriverInput } from '@rideshare/validation';
...
interface AuthContextValue extends AuthState {
  ...
  logout: () => Promise<void>;
  /** Re-reads /auth/me after an action that changes `user`-visible state
   * server-side (e.g. submitting an application) without a fresh login. */
...
/**
 * Owns the driver's signed-in state for the whole app — the same design
 * as apps/passenger-app's AuthContext (Phase 3), applied here: resume a
 * session from SecureStore on mount, verify via GET /auth/me, fall back
 * to one refresh attempt, then give up and sign out. Screens never call
 * apiClient's auth functions directly.
 */
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const register = useCallback(async (input: RegisterDriverInput) => {
    const response = await apiClient.registerDriver(input);
    await setStoredTokens(response.tokens);
    setState({ status: 'signedIn', user: response.user, accessToken: response.tokens.accessToken });
  }, []);
