 ...
 * first, and lets a passenger mark one (or everything) read. No
 * per-type icon/label mapping: the 11 canonical event types
 * (@rideshare/types's NotificationType) are a stable machine-matchable
 * key for future deep-linking, not something this screen needs to
 * branch on today.
 */
export function NotificationsScreen() {
  ...
  // below — same react-hooks/set-state-in-effect workaround used by
  // driver-app's DocumentsScreen: the actual fetch stays inline in the
...
const styles = StyleSheet.create({
  ...
  cardBody: { color: '#cbd5e1', fontSize: 13, marginTop: 6 },
