...
export default function VehiclesPage() {
  ...
  useEffect(() => {
    ...
    async function load(token: string) {
      try {
        ...
        if (!cancelled) {
          setErrorMessage(error instanceof ApiClientError ? error.message : 'Could not load vehicles.');
        }
      }
    }
  ...
  return (
    <AdminShell title="Vehicles" subtitle={`${vehicles.length} vehicle(s)`} errorMessage={errorMessage}>
      {vehicles.length === 0 ? (
            ...
            {vehicles.map((vehicle) => (
              <tr key={vehicle.id} className="border-t border-slate-100">
                <td className="px-4 py-3">
                  <Link href={`/drivers/${vehicle.driverId}`} className="font-medium text-slate-900 hover:underline">
                    {vehicle.driverName}
                  </Link>
                </td>
                ...
                <td className="px-4 py-3">
                  <span
                    className={`rounded-full px-2 py-1 text-xs font-semibold ${
                      vehicle.isActive ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-600'
                    }`}
                  >
